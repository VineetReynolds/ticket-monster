...
* HTML5 & AngularJS for building an mobile web front-end.  
...
`test/java`::
    provides the `org.jboss.examples.ticketmonster.test` package that contains `MemberRegistrationTest.java`, an Arquillian based test that runs both from within JBoss Developer Studio via *Run As -> JUnit Test* and at the command line:
+
